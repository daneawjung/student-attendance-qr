...
async function load(){
 ...
 try{
  ...
  for(const s of schedules){
   ...
   const checked=counts.present+counts.late+counts.leave;
  ...
  list.innerHTML=rows.map(row=>{
   ...
   return `<div class="focus-card ${current?'current':''}">
    <div class="focus-main"><div class="today-time">${esc(s.start_time)}–${esc(s.end_time)} ${current?'⏰':''}</div><strong>${esc(s.subject_name)}</strong><div class="focus-meta">${esc(s.subject_code)} • 👥 ${esc(s.class_name)}${s.room?' • 🏫 '+esc(s.room):''}</div><div class="focus-meta ${open?'session-open':''}">${statusText}</div></div>
    <div class="attendance-summary"><div class="summary-total">👨‍🎓 ${total} คน</div><div class="summary-chips"><span>🟢 ${c.present}</span><span>🟡 ${c.late}</span><span>🔵 ${c.leave}</span><span>🔴 ${absent}</span></div><div class="summary-progress"><span style="width:${percent}%"></span></div><small>เช็กแล้ว ${row.checked}/${total} คน</small></div>
    <div>${button}</div>
   </div>`;
  }).join('');
