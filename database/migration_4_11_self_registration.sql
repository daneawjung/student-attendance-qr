...
alter table attendance_subjects drop constraint if exists attendance_subjects_max_students_check;
alter table attendance_subjects add constraint attendance_subjects_max_students_check
...
begin
  ...
  if not v_enrolled then
    select count(*) into v_count from attendance_subject_enrollments where subject_id=v_subject.id and status='active';
  ...
  return json_build_object('student_id',v_student.id,'student_code',v_student.student_id,'prefix',v_student.prefix,
    'first_name',v_student.first_name,'last_name',v_student.last_name,'class_name',v_student.class_name,'enrolled',true);
end;
$$;
