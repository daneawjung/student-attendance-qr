...
async function load(){
  ...
  try{
    ...
    const schedulesResult=await supabaseClient.from('attendance_weekly_schedules')
      .select('id,start_time,end_time,subject_code,subject_name,class_name,room,teacher_name,active')
      .eq('day_of_week',day).eq('active',true).order('start_time');
    if(schedulesResult.error)throw schedulesResult.error;

    const sessionsResult=await supabaseClient.from('attendance_sessions')
      .select('id,session_code,subject_code,subject_name,class_name,start_time,end_time,status')
    ...
    for(const s of schedules){
      ...
      let count=0;
      if(session){
        const r=await supabaseClient.from('attendance_records').select('id',{count:'exact',head:true}).eq('session_id',session.id).eq('status','present');
    ...
    list.innerHTML=rows.map(function(row){
      const s=row.s,session=row.session,count=row.count;
      ...
      const button=open
        ?`<a class="focus-button" href="session-control.html?session=${encodeURIComponent(session.id)}">📋 ดูรายชื่อ</a>`
        :`<a class="focus-button" href="today.html">▶️ ${session?'ดูคาบ':'เปิดเช็กชื่อ'}</a>`;
    ...
    list.innerHTML=`<div class="empty-state">⚠️ โหลดข้อมูลไม่ได้<br><small>${esc(err.message||err)}</small></div>`;
  }
}
load();
